...
public class MinimumDepthOfBinaryTree_111 {
    ...
    public int minDepthDFS(TreeNode root) {
         ...
         * We CANNOT use Math.min(left, right) + 1 here directly,
         * because the missing child would return 0 and incorrectly give depth 1.
         * We must explicitly go down the ONLY existing path.
         */
        if (root.left == null) {
            return minDepthBFS(root.right) + 1; // Must go right
        }

        if (root.right == null) {
            return minDepthBFS(root.left) + 1; // Must go left
        }

        // Node has two children - safe to take the minimum of both
        return Math.min(minDepthBFS(root.left), minDepthBFS(root.right)) + 1;
    }
